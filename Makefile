...
.PHONY: docker
docker :
	docker build . -f build/service.Dockerfile --build-arg SERVICE_NAME=cart --build-arg SERVICE_PORT=9001 -t $(DOCKER_PREFIX)/cart:$(DOCKER_TAG)
	docker build . -f build/service.Dockerfile --build-arg SERVICE_NAME=products --build-arg SERVICE_PORT=9002 --build-arg CGO_ENABLED=1 -t $(DOCKER_PREFIX)/products:$(DOCKER_TAG)
	docker build . -f build/service.Dockerfile --build-arg SERVICE_NAME=users --build-arg SERVICE_PORT=9003 -t $(DOCKER_PREFIX)/users:$(DOCKER_TAG)
	docker build . -f build/service.Dockerfile --build-arg SERVICE_NAME=orders --build-arg SERVICE_PORT=9004 -t $(DOCKER_PREFIX)/orders:$(DOCKER_TAG)
	docker build . -f build/frontend.Dockerfile -t $(DOCKER_PREFIX)/frontend-host:$(DOCKER_TAG)
